...
class IDA(AStar):
    def solve(self):
        ...
        state = (self.initial_board.player_pos, self.initial_board.boxes)
        path_stack = deque()
        ...
        while True:
            t = self._search(path_stack, path_moves, set(), 0, bound)
            if t == 0:
    ...
    def _search(self, path_stack: deque, path_moves: deque, path_visited: set, g, bound):
        self.iteration += 1
        ...
        for new_pos, new_boxes, move in self._expand_moves(player_pos, boxes):
            if (new_pos, new_boxes) not in path_visited:
                ...
                t = self._search(path_stack, path_moves, path_visited, g + 1, bound)
                if t == 0:
                    return t
                if t < minimum:
                    minimum = t
